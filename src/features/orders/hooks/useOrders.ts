...
export function useAddOrderComment() {
  ...
  return useMutation({
    mutationFn: async ({ orderId, content }: { orderId: string; content: string }) => {
      ...
      // Get order for org_id and notification targets
      const { data: order } = await supabase
        .from('orders' as any)
        .select('organization_id, created_by, assigned_to, title, code')
        .eq('id', orderId)
        .single();
      const o = order as Record<string, any>;
      if (!o?.organization_id) throw new Error('Pedido não encontrado');
