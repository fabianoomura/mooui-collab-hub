...
async function syncLaunchToCalendar(launch: any) {
  if (!launch.start_date) return;
...
export function useCreateLaunch() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data) syncLaunchToCalendar(data).then(() => qc.invalidateQueries({ queryKey: ['calendar-events'] }));
    },
  });
}
    ...
    mutationFn: async (id: string) => {
      if (currentOrg) {
        await supabase.from('calendar_events' as any).delete()
          .eq('source_type', 'launch').eq('source_id', id).eq('organization_id', currentOrg.id);
      }
