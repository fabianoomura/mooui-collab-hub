...
export function useTicketActivity(ticketId: string | null) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Map polymorphic → legacy shape
      return (data || []).map((a: any) => ({
        id: a.id,
        ticket_id: a.entity_id,
        user_id: a.actor_id,
        action: a.action,
        from_value: a.payload?.from ?? null,
        to_value: a.payload?.to ?? null,
        created_at: a.created_at,
      })) as unknown as TicketActivity[];
    },
    enabled: !!ticketId,
  });
}
...
export function useAddTicketComment() {
  ...
  return useMutation({
    mutationFn: async ({ ticketId, content }: { ticketId: string; content: string }) => {
      ...
      // Get ticket for org_id and notification targets
      const { data: t } = await supabase.from('tickets')
        .select('organization_id, title, created_by, assigned_to').eq('id', ticketId).single();
