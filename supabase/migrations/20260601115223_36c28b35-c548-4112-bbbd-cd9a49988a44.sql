...
', updated_at = now() WHERE title ILIKE '%Sunday%';
...
', updated_at = now() WHERE title ILIKE '%Configurações%';
...
', updated_at = now() WHERE title ILIKE '%Tickets%';
...
', updated_at = now() WHERE title ILIKE '%Salas%';
...
', updated_at = now() WHERE title ILIKE '%Equipe%';
...
', updated_at = now() WHERE title ILIKE '%Speaks%';
...
<li>Dê título, ícone e escreva no editor rico</li>
</ol>
<h2>Hierarquia</h2>
...
', updated_at = now() WHERE title ILIKE '%Papelinho%';
...
', updated_at = now() WHERE title ILIKE '%Eventos%';
...
<p>Base de conhecimento da empresa. Crie páginas com editor rico (títulos, listas, tabelas, imagens, checklists), organize por setor e defina permissões.</p>
...
', updated_at = now() WHERE title ILIKE '%Manual de Uso%';
...
', updated_at = now() WHERE title ILIKE '%Pedidos%';
...
', updated_at = now() WHERE title ILIKE '%Lançamentos%';